...
class SliderGenerator implements DemoGeneratorInterface {
  ...
  public static function generate($variation, $values) {
    ...
    $page = LayoutNodeHelper::createNodeWithLayoutVariation('page', $target_field, 'Sliders - ', $variation);
    ...
    foreach($layout_paragraphs as $layout_paragraph_index => $layout_paragraph) {
      ...
      foreach($paragraph_regions as $region_name => $region_label) {
        ...
        // We really don't want to generate a zillion sliders in tiny columns here.
        if(!in_array($paragraph_layout_name, ['m3_one_column', 'm3_two_column_a', 'm3_two_column_b', 'm3_two_column_c'])) {
          continue;
        }
        ...
        switch($populate_with) {
          case 'heading':
            ...
            break;
          case 'slider':
            $content_paragraphs[] = ParagraphsHelper::createSliderParagraph($region_name, $paragraph_uuid);
            break;
        }
      }
    }
